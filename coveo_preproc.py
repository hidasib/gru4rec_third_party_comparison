...
def main(path:str):
    data = pd.read_csv(f'{path}/release_10_23_2020.csv', sep=',', date_parser=['server_timestamp_epoch_ms'], dtype={'session_id_hash':str, 'event_type':'category', 'product_action':str, 'product_skus_hash':str, 'hashed_url':str})
    session_hashes = data.session_id_hash.unique()
    item_hashes = data.product_skus_hash.unique()

    session_hash_start = data.groupby('session_id_hash', as_index=False).agg(Start=('server_timestamp_epoch_ms', 'min'))
    session_hash_start.sort_values(by=['Start'], inplace=True)
    session_hash_start['session_idx'] = np.arange(session_hash_start.session_id_hash.nunique())

    data = pd.merge(data, session_hash_start, on=['session_id_hash'])
    data = pd.merge(data, pd.DataFrame({'product_skus_hash': item_hashes, 'item_id': np.arange(len(item_hashes))}))
    ...
    data["session_id"] = np.cumsum(
        np.hstack(
            [
                False,
                (data.server_timestamp_epoch_ms.values[1:] - data.server_timestamp_epoch_ms.values[:-1] > 3600 * 1000)
    ...
    view = pd.DataFrame({'session_id': view.session_id, 'time': view.server_timestamp_epoch_ms, 'item_id': view.item_id})
    ...
    tsplit = 1545782400 * 1000# this is the end of the last full day
    tday = 86400 * 1000
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--path", type=str, help="Path of the directory of the dataset", required=True)
